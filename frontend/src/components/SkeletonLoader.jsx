...
const SkeletonLoader = ({ variant = 'list', count = 3 }) => {
  const PulseBlock = ({ className }) => (
    <div className={`animate-pulse bg-slate-800/60 light:bg-slate-200/80 rounded-xl ${className}`} />
  );
  ...
  switch (variant) {
      ...
      return (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 w-full">
          {Array.from({ length: 4 }).map((_, idx) => (
            <div key={idx} className="bg-slate-900 light:bg-white border border-slate-850 light:border-slate-200 p-5 rounded-2xl shadow-lg space-y-3">
    ...
    case 'table':
      return (
        <div className="bg-slate-900 light:bg-white border border-slate-850 light:border-slate-200 p-6 rounded-2xl shadow-xl space-y-4 w-full">
          <PulseBlock className="h-6 w-1/4 mb-4" />
          <div className="space-y-3">
            {/* Table Header skeleton */}
            <div className="flex gap-4 border-b border-slate-800 light:border-slate-200 pb-3">
      ...
      return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full">
          {Array.from({ length: 3 }).map((_, colIdx) => (
            <div key={colIdx} className="bg-slate-900 light:bg-white border border-slate-850 light:border-slate-200 rounded-2xl p-4 flex flex-col space-y-4 min-h-[400px]">
              {/* Column Header */}
              ...
              {Array.from({ length: 2 }).map((_, cardIdx) => (
                <div key={cardIdx} className="bg-slate-955 light:bg-slate-50 border border-slate-850 light:border-slate-200 p-4 rounded-xl space-y-3">
    ...
    case 'chart':
      return (
        <div className="bg-slate-900 light:bg-white border border-slate-850 light:border-slate-200 p-5 rounded-2xl shadow-lg space-y-3 w-full h-[320px] flex flex-col justify-between">
          ...
          <div className="flex justify-between pt-2 border-t border-slate-850/60 light:border-slate-200">
    ...
    default:
      return (
        <div className="space-y-4 w-full">
          {Array.from({ length: count }).map((_, idx) => (
            <div key={idx} className="bg-slate-900 light:bg-white border border-slate-855 light:border-slate-200 p-4 rounded-2xl flex items-center justify-between shadow-sm">
