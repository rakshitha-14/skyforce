      ...
      <div className="space-y-8">
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-xl">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">{user?.designation} • {user?.department} Department</p>
          </div>
          {/* Quick status pill */}
          <div className="flex items-center gap-3">
            <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Clock Status:</span>
            {!clockStatus.checkedIn ? (
              ...
              <span className="bg-slate-950 border border-slate-800 text-slate-400 px-3 py-1 rounded-full text-xs font-bold">
                Shift Completed
              </span>
            )}
          </div>
        ...
        {activeTab === 'dashboard' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1 bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl flex flex-col justify-between h-fit space-y-6">
              <div>
                <h2 className="text-lg font-bold text-slate-200 mb-2">Punch Attendance</h2>
                <p className="text-slate-400 text-sm">Log your check-in and check-out times here daily.</p>
              </div>

              {message && (
                <div className="p-3 bg-blue-950/40 border border-blue-900/60 text-blue-400 text-xs text-center rounded-lg">
                  {message}
                </div>
              )}

              <div className="space-y-4">
                <div className="flex justify-between items-center bg-slate-950 p-4 rounded-xl border border-slate-800/50">
                  <span className="text-xs text-slate-400 font-semibold uppercase">Punch In</span>
                  <span className="text-sm font-mono text-slate-200">
                    {clockStatus.record ? new Date(clockStatus.record.checkIn).toLocaleTimeString() : '--:--'}
                  </span>
                </div>
                <div className="flex justify-between items-center bg-slate-950 p-4 rounded-xl border border-slate-800/50">
                  <span className="text-xs text-slate-400 font-semibold uppercase">Punch Out</span>
                  <span className="text-sm font-mono text-slate-200">
            ...
            <div className="lg:col-span-2 bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl flex flex-col justify-between space-y-6">
              <div>
                <h2 className="text-lg font-bold text-slate-200 mb-2">Shift & Tasks Summary</h2>
                <p className="text-slate-400 text-sm">Key statistics for your current performance.</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-950 p-5 rounded-xl border border-slate-800/50">
                  <span className="text-xs text-slate-400 uppercase font-semibold">Total Assigned Tasks</span>
                  <div className="text-3xl font-extrabold text-slate-200 mt-2">{tasks.length}</div>
                </div>
                <div className="bg-slate-950 p-5 rounded-xl border border-slate-800/50">
              ...
              <div className="bg-slate-950 p-5 rounded-xl border border-slate-800/50 flex items-center justify-between">
                <div>
                  <span className="text-xs text-slate-400 uppercase font-semibold">Today's Shift Hours</span>
        ...
        {activeTab === 'tasks' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center bg-slate-900 border border-slate-800 p-4 rounded-2xl shadow-md">
              <div>
                <h2 className="text-lg font-bold text-slate-200">My Assigned Tasks</h2>
                <p className="text-slate-400 text-sm">Update the workflow status of your tickets.</p>
              </div>
            ...
            {tasksViewMode === 'kanban' ? (
              ...
              <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl flex flex-col justify-between space-y-6">
                <div>
                  <h2 className="text-lg font-bold text-slate-200 mb-2">My Assigned Tasks</h2>
                  <p className="text-slate-400 text-sm">Update the workflow status of your tickets.</p>
                </div>

                {tasks.length === 0 ? (
                  <div className="text-center py-12 text-slate-500 bg-slate-950 rounded-xl border border-slate-800/50">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 overflow-y-auto max-h-[400px] pr-2">
                    {tasks.map(task => (
                      <div
                        key={task._id}
                        className="bg-slate-950 border border-slate-800 p-5 rounded-xl flex flex-col justify-between space-y-4 hover:border-slate-700 transition-colors"
                      >
                        <div className="space-y-2">
                          ...
                          <h3 className="text-sm font-bold text-slate-200">{task.title}</h3>
                            ...
                            <div className="mt-2 flex flex-wrap gap-2">
                              {task.attachments.map((att, i) => (
                                <a 
                                  ...
                                  target="_blank" 
                                  rel="noreferrer" 
                                  className="text-[10px] bg-slate-900 border border-slate-805 text-blue-400 hover:underline px-2 py-1 rounded flex items-center gap-1.5 cursor-pointer animate-fade-in"
                                >
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-slate-900">
                          ...
                          <select
                            ...
                            className={`text-xs bg-slate-900 border border-slate-800 rounded-lg px-2.5 py-1.5 focus:outline-none focus:border-blue-500 cursor-pointer ${
                          ...
                          >
                            <option value="To Do" className="text-slate-400">To Do</option>
                            <option value="In Progress" className="text-blue-400">In Progress</option>
                            <option value="In Review" className="text-purple-400">In Review</option>
        ...
        {activeTab === 'logs' && (
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl space-y-4">
            <div>
              <h2 className="text-lg font-bold text-slate-200">Attendance Log History</h2>
              <p className="text-slate-400 text-sm">Review your historical shift logs and total hours.</p>
            </div>

            {logs.length === 0 ? (
              <div className="text-center py-6 text-slate-500">No attendance entries recorded.</div>
            ) : (
